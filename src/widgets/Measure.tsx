...
export default class Measure extends Widget {
  ...
  private _addMarkerGraphic(vertex: [number, number]): void {
    this._layer.add(
      new Graphic({
        geometry: new Point({
          x: vertex[0],
  ...
  private _addLineGraphic(geometry: Polyline): void {
    this._layer.addMany([
      new Graphic({
        geometry,
  ...
  private _addFillGraphic(geometry: Polygon): void {
    this._layer.add(
      new Graphic({
        geometry,
  ...
  private _addTextGraphic(geometry: Point | Polyline | Polygon, text: string): void {
  ...
  private _lengthEvent(evt: any): void {
    ...
    polyline.paths[0].forEach((vertex: number[]) => {
      this._addMarkerGraphic(vertex as [number, number]);
    });
  ...
  private _areaEvent(evt: any): void {
    ...
    polygon.rings[0].forEach((vertex: number[]) => {
      this._addMarkerGraphic(vertex as [number, number]);
    });
  ...
  //
  // --------------------------------------------------------------------------
  private _centerLocation(center: Point): void {
  ...
  private _identifyLocation(): void {
    this._clear();
  ...
  private _locationEvent(evt: any): void {
  ...
  //
  // --------------------------------------------------------------------------
  private _centerElevation(center: Point): void {
  ...
  private _identifyElevation(): void {
    this._clear();
  ...
  private _elevationEvent(evt: any): void {
    const x = evt.vertices[0][0];
    const y = evt.vertices[0][1];
    const spatialReference = this.view.spatialReference;
    const point = new Point({
