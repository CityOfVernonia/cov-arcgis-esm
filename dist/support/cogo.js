 ...
 * @returns esri.Point
 */
export const linearInterpolation = (point1, point2, linearDistance) => {
 ...
 * @returns esri.Point
 */
export const midpoint = (polyline) => {
    ...
    for (let i = 0; i < segements.length - 1; i += 1) {
        if (dsf + distance(new Point({ ...segements[i] }), new Point({ ...segements[i + 1] })) > td / 2) {
            ...
            return linearInterpolation(new Point({ ...segements[i], spatialReference }), new Point({ ...segements[i + 1], spatialReference }), distanceToMidpoint);
        }
